  ...
  Future<SttTranscriptionResult?> transcribe(
    ...
    try {
      ...
      try {
        ...
        // Self-host patch: silence (or noise-only markers like [BLANK_AUDIO]) is a
        // SUCCESSFUL transcription with no text, not a failure — return an empty
        // result, never null. Our PurePollingSocket treats null as "the attempt
        // failed, requeue and retry the same audio"; returning null here made plain
        // silence look like an outage (offline indicator on, silent frames requeued
        // and re-transcribed forever).
        if (res.text.isEmpty) {
        ...
        // Self-host patch: same contract — nothing left after stripping markers
        // is silence, not a failed attempt.
        ...
        if (filteredText == null) {
          ...
          // Self-host patch: отбраковка — это УСПЕШНО распознанное «ничего», а не
          // провал попытки. Upstream отдаёт здесь null, потому что у него null не
          // означает «повтори»; у нас означает (см. PurePollingSocket), и тот же
          // кусок аудио уходил бы в бесконечный перезапрос с индикатором офлайна.
          return SttTranscriptionResult(segments: const []);
        }
