...
def test_session_scoped_message_reads_have_a_declared_composite_index(monkeypatch):
    ...
    Regression for a self-host FailedPrecondition 400 on POST /v2/messages and
    GET /v2/messages?chat_session_id=...: a chat session's first page of messages hits this
    branch instead of the app-scoped one covered above, and it 500s independently of it.
    """
    recorder = []
    monkeypatch.setattr(chat_db, 'db', _StreamRecordingFirestore(recorder, collection_name='messages'))
