...
def get_conversations_without_photos(
    ...
    # Здесь встречаются две независимые работы, и обе нужны: бюджет чтения
    # upstream (#11831) и наш self-host фолбэк на отсутствующий композитный
    # индекс. Порядок важен — сначала штатный путь под бюджетом, и только
    # отсутствие индекса уводит в обход.
    conversations = []
    try:
        ...
        # Self-host patch: only the unfiltered recent-list shape is missing its
        # index; every other filter combination here has one declared, so a 400
        # there is a real gap worth surfacing.
        if not _is_plain_recent_conversations_query(
            include_discarded, statuses, categories, folder_id, starred, start_date, end_date, sources
        ):
            raise
        logger.warning(f"get_conversations missing_composite_index uid={uid} falling back to client-side filter")
        # Обходной путь не проходит через budgeted_stream_iter — он читает
        # напрямую, поэтому строки, прочитанные до отказа, отбрасываются: иначе
        # в списке оказались бы две пересекающиеся выборки.
        conversations = [
            _document_data_with_revision(doc)
            for doc in _recent_conversations_without_composite_index(uid, limit, offset)
        ]
    conversations = [conversation for conversation in conversations if conversation is not None]
    return conversations
