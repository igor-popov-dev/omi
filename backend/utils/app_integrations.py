...
def _process_mentor_proactive_notification(uid: str, conversation_messages: list[dict]) -> str | None:
    ...
    #
    # The two sources fail independently on purpose. Semantic search needs an embedding
    # provider and a vector store; recent-by-time needs neither. Sharing one try/except
    # meant a single embedding failure (no key, quota, outage) also dropped the recent
    # conversations fetched right after it, leaving the mentor with no past context at all
    # — silently, since the draft still gets written from the live transcript alone.
