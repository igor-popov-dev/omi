...
if _auth_emulator_host:
    # OMI_LOCAL_DEV_REAL_FIRESTORE keeps ADC credentials alive so the data layer
    # can target a real Firestore project while sign-in stays on the local Auth
    # emulator. Without the explicit opt-in, emulator auth still strips every
    # real-credential selector so a local instance cannot touch cloud data.
    if not os.environ.get("OMI_LOCAL_DEV_REAL_FIRESTORE", "").strip():
